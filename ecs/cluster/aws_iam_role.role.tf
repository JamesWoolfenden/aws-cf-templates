...
resource "aws_iam_role_policy" "ecsYukmSB" {
  name="ecsYukmSB"
  role=aws_iam_role.Role.id
  policy = jsonencode({"Statement":[{"Action":["ecs:DiscoverPollEndpoint"],"Effect":"Allow","Resource":"*"}],"Version":"2012-10-17"})
}

resource "aws_iam_role_policy" "ecs-clusterYukmSB" {
  name="ecs-clusterYukmSB"
  role=aws_iam_role.Role.id
...
resource "aws_iam_role_policy" "ecs-cluster-instanceYukmSB" {
  name="ecs-cluster-instanceYukmSB"
  role=aws_iam_role.Role.id
...
resource "aws_iam_role_policy" "ecrYukmSB" {
  name="ecrYukmSB"
  role=aws_iam_role.Role.id
...
resource "aws_iam_role_policy" "logsYukmSB" {
  name="logsYukmSB"
  role=aws_iam_role.Role.id
