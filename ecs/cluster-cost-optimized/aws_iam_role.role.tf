...
resource "aws_iam_role_policy" "ecsXNNxKM" {
  name="ecsXNNxKM"
  role=aws_iam_role.Role.id
  policy = jsonencode({"Statement":[{"Action":["ecs:DiscoverPollEndpoint"],"Effect":"Allow","Resource":"*"}],"Version":"2012-10-17"})
}

resource "aws_iam_role_policy" "ecs-clusterXNNxKM" {
  name="ecs-clusterXNNxKM"
  role=aws_iam_role.Role.id
...
resource "aws_iam_role_policy" "ecs-cluster-instanceXNNxKM" {
  name="ecs-cluster-instanceXNNxKM"
  role=aws_iam_role.Role.id
...
resource "aws_iam_role_policy" "ecrXNNxKM" {
  name="ecrXNNxKM"
  role=aws_iam_role.Role.id
...
resource "aws_iam_role_policy" "logsXNNxKM" {
  name="logsXNNxKM"
  role=aws_iam_role.Role.id
